...
export default function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    onPaginationChange: paginate ? setPagination as React.Dispatch<React.SetStateAction<PaginationState>> : undefined,
  });

  const PaginationComponent = ({ pagination }: { pagination: PaginationState }) => {
  ...
  return (
    <div className="w-full">
      <div
        className={`relative w-full grid grid-cols-2 ${filtering || ActionComponent ? "mb-2" : ""
          }`}
      >
